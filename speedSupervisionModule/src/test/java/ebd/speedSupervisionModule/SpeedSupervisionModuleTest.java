...
class SpeedSupervisionModuleTest {
    ...
    public void ssmTest() throws InterruptedException {
        ...
        TrainData trainData = new TrainData(eventBus, "C:\\intellij-workspace\\etcs\\trainData\\test650.json");
        eventBus.post(new TrainDataChangeEvent("test", Arrays.asList("td"),"currentSpeed", 51d));
        ...
        bcre.source = "test";
        bcre.targets = Arrays.asList("bcc");
        eventBus.post(bcre);

        Thread.sleep(8000);
        eventBus.post(new DisconnectEvent("all", Arrays.asList("all")));
    }
