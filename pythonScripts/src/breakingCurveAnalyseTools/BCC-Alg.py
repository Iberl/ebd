...
def numBackIntegration(d_EMA, v_EMA, s_EAP):
    ...
    while s_n > s_EAP:
        ...
        dv = DV_INIT
        while v_n < v_TSP(s_n) and s_n > s_EAP:                 #B) Needed: Additional test if s_n > s_EAP in case v_n does not reach v_TSP(s_EAP)
        ...
        Knots.append(s_n1)
        c[0].append(v_n1)                                       #A) Needed to be v_n1
        c[1].append(0)
        n += 1
        v_n = v_n1 - 0.001                                      #D) To allow the second loop to be started again, else, v_n < v_TSP(s_n) will never be true again
        s_n = s_n1 
