...
public class Main {
    ...
    public static List<Balise> getBalisesFromFile(String sFile) throws JAXBException {
        ...
        return getBalises(expenseObj);
    }

    public static List<Balise> getBalises(CPlanProSchnittstelle expenseObj) {
        handleFileData(expenseObj);
        List<Balise> baliseList = extractBaliseData();
        if(baliseList.size() > 0) {
            Balise B = baliseList.get(0);

            //System.out.println(new ReflectionToStringBuilder(B, new MultilineRecursiveToStringStyle()).toString());
        }
        return baliseList;
    }
