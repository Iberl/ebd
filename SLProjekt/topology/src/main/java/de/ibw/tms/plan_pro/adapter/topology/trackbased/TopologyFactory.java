...
public class TopologyFactory implements ITopologyFactory {
    ...
    public static final String MARTINSTEIN_LAST_PPXML = "customised/EBD_Martinstein_last.ppxml";
    /**
        ...
        for(Balise B : balisesList) {
            ...
            try {
                ...
                if(isAMissing && isBMissing) {
                    ...
                    geoCoordinate = UtilFunction.getGeoCoordinate(TopKante.getIdentitaet().getWert(), true, decDistanceFromA.doubleValue());
                    ...
                    geoCoordinate = UtilFunction.getGeoCoordinate(TopKante.getIdentitaet().getWert(), !isAMissing, decResult.doubleValue());
                }


            } catch(Exception Ex) {
                geoCoordinate = UtilFunction.getGeoCoordinate(TopKante.getIdentitaet().getWert(), true, dA);
            }
    ...
    private CPlanProSchnittstelle getcPlanProSchnittstelle() throws JAXBException {
        ...
        JAXBElement<CPlanProSchnittstelle> unmarshalledObject =
                (JAXBElement<CPlanProSchnittstelle>)unmarshaller.unmarshal(
                        ClassLoader.getSystemResourceAsStream(MARTINSTEIN_LAST_PPXML));
